...
export class PncpApi {
  ...
  search(terms: string[] | undefined, page: number, pageSize: number, status?: string): Promise<unknown> {
    return this.http.json(PncpApi.searchUrl, { tipos_documento: 'ata', q: terms?.join(' '), pagina: page, tam_pagina: pageSize, status }, 'busca textual');
  }
